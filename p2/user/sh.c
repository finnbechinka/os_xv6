...
runcmd(struct cmd *cmd)
{
  ...
  case EXEC:
    ...
    /* try to exec file in current folder */
    ...
    /* try to exec file in root folder */
    exec(rp, ecmd->argv);
    ...
    close(fd2);
    /* print error if file doesnt exists in current or root folder */
    if(fd1 == -1 && fd2 == -1){
      fprintf(2, "file not found in current nor root folder\n");
    }

    /* print error if exec in both current and root folder failed */
